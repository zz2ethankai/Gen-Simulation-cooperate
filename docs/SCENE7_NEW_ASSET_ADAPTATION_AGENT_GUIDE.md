...
- `rotate_to_heading_enabled` 除非已做 runtime 验证，否则显式保持 `false`。
...
- 该轮还有一个重要配置生命周期陷阱：goal 目录下冻结的 `debug_inputs/split_aloha_nav2_skill_params.yaml` 已经是 `FollowPath.plugin: nav2_mppi_controller::MPPIController`，但实际 resident Nav2 容器启动时使用的 `output/nav2_runtime/nav2_scene8val/bootstrap/nav2_params.yaml` 仍是 `FollowPath.plugin: nav2_rotation_shim_controller::RotationShimController`。如果只看 goal debug params，会误判 `rotate_to_heading_enabled: false` 已经控制住 live Nav2；排查时必须同时看 resident bootstrap params 和 nav2 容器日志。
- 因此 scene_8 验证前要保证 resident bootstrap 默认也不会启用 Rotation Shim。当前代码已把 `workflows/simbox/core/configs/nav/default_nav.yaml` 的 `rotate_to_heading_enabled` 默认改为 `false`；重跑时必须重启 Nav2 容器并确认 `output/nav2_runtime/<session>/bootstrap/nav2_params.yaml` 里的 `controller_server.ros__parameters.FollowPath.plugin` 是 `nav2_mppi_controller::MPPIController`，否则本轮修复没有进入实际控制器。
...
- 这轮还暴露两个修复点：一是 skill YAML 写了 `xy_goal_tolerance/yaw_goal_tolerance: 0.1`，但旧 `Navigate._nav2_skill_overrides()` 只下推 `rotate_to_heading_enabled`，live `debug_inputs/*nav2_skill_params.yaml` 仍用 default goal checker `0.06`；后续 agent 应确认 skill 级 tolerance 已写进 `controller_server.general_goal_checker`。二是 `approach_footprint_padding: 0.02` 小于 live global/local costmap padding；该 index `137` 目标点在 `0.02m` 静态检查下可行，但用 `0.03m` padding 已有 blocked padding cells，用 local `0.10m` padding 更不安全。参考任务应把 approach padding 调到和 local costmap 一致，或至少不小于 global footprint padding。
