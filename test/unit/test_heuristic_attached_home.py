...
def test_home_interpolation_emits_direct_commands_and_exact_endpoint():
    skill = SimpleNamespace(move_steps=3, _gripper_state=-1.0, t_eps=0.088)
    ...
    assert all(command[2] == "dummy_forward" for command in commands)
    np.testing.assert_allclose(commands[0][3]["arm_action"], [1 / 3, 5 / 3])
    np.testing.assert_allclose(commands[-1][3]["arm_action"], [1.0, 3.0])
...
def test_heuristic_home_contains_no_physics_schema_carry_planner():
    ...
    assert '"dummy_forward"' in source
