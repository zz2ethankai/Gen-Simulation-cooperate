...
class ControllerExecution:
    ...
    def __init__(
        ...
        *,
        ...
        self._gripper_action = self._canonical_gripper_action()
        if self.state.gripper_joint_position is None:
            command = (
                self._gripper_action["command"]
                if self._gripper_action is not None
                else getattr(self.arm_spec, "gripper_home", (1.0,))
            )
    ...
    def _canonical_gripper_action(self):
        """Return the canonical signed-clip action for this controller arm."""

        robot_cfg = getattr(self.robot, "cfg", None)
        if not isinstance(robot_cfg, Mapping) or self.lr_name not in {"left", "right"}:
            return None
        arms = robot_cfg.get("arms")
        arm = arms.get(self.lr_name) if isinstance(arms, Mapping) else None
        gripper = arm.get("gripper") if isinstance(arm, Mapping) else None
        action = gripper.get("action") if isinstance(gripper, Mapping) else None
        if not isinstance(action, Mapping):
            return None
        adapter = str(action.get("adapter", ""))
        if adapter != "signed_clip":
            raise ValueError(
                f"unsupported canonical gripper action adapter: {adapter!r}"
            )
        command = np.asarray(action.get("command", ()), dtype=float).reshape(-1)
        clip = np.asarray(action.get("clip", ()), dtype=float).reshape(-1)
        expected_size = int(self.gripper_indices.size)
        if command.size != expected_size or not np.all(np.isfinite(command)):
            raise ValueError(
                "canonical gripper action command must contain one finite value "
                "per gripper joint"
            )
        if (
            clip.size != 2
            ...
            raise ValueError("canonical gripper action clip must be [min, max]")
        return {
            "command": command,
    ...
    def get_gripper_action(self):
        if self._gripper_action is not None:
            sign = -1.0 if self._gripper_action["invert"] else 1.0
            scale = 1.0
            clip_min, clip_max = self._gripper_action["clip"]
        else:
            spec = self.arm_spec
