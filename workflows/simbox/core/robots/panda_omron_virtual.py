...
class PandaOmronVirtual(PandaOmron):
    ...
    def _configure_mobile_base_wheel_drives(self):
        ...
        drive_specs = (
            ("linear", linear_drive_damping, linear_drive_max_force),
            ("linear", linear_drive_damping, linear_drive_max_force),
            ("angular", yaw_drive_damping, yaw_drive_max_force),
        )
        for joint_path, (drive_name, damping, max_force) in zip(self._wheel_joint_paths, drive_specs):
    ...
    def _reset_virtual_base_joint_state(self, *, require_ready: bool):
        ...
        self._articulation_view.set_joint_velocity_targets(zero, joint_indices=base_joint_indices)
    ...
    def apply_base_command(self, steering_positions, wheel_velocities, *, step_dt: float):
        ...
        next_positions = virtual_positions + world_velocity * dt
        next_positions[2] = self._wrap_angle(float(next_positions[2]))
        next_positions = next_positions.reshape(1, -1)
