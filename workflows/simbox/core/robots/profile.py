...
class RobotBaseProfile:
    ...
    # Optional mobile-navigation fields (base_config_file, wheel_joint_names,
    # local_navigation, ...).  Fixed/table bases simply omit them.
...
class RobotArmProfile:
    ...
    # Optional runtime fields (base_mount_translation, arm/gripper drive
    # gains, ...) carried verbatim and projected back as flat runtime keys.
...
# Optional mobile-navigation fields allowed inside base for mobile robots.
# Fixed/table bases omit them; load_robot_profile tolerates and preserves them.
...
def project_runtime_config(
    ...
    *,
    ...
    if "mobile_base_path" in profile.base.navigation:
        config["mobile_base_path"] = str(profile.base.navigation["mobile_base_path"])
    ...
    for arm_id, arm in profile.arms.items():
        ...
        if arm.gripper.joint_names:
            ...
            config[f"{long_prefix}_gripper_joint_names"] = list(arm.gripper.joint_names)
