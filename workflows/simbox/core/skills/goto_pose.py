...
class Goto_Pose(BaseSkill):
    ...
    def simple_generate_manip_cmds(self):
        ...
        if self.q_base_ee_tgt is None:
            ...
            # Candidate feasibility belongs to the typed controller runtime;
            # execution plans the selected target from its measured state.
            self.p_base_ee_tgt, self.q_base_ee_tgt = p_base_ee_tgts[0], q_base_ee_tgts[0]
            ...
            for interp_trans, interp_ori in zip(interp_trans_list, interp_ori_list):
                manip_list.append(
                    self.pose_command(
                        ...
                        interp_trans,
                        interp_ori,
                        gripper_action=self.gripper_fn,
                    )
                )
        else:
            manip_list.append(
