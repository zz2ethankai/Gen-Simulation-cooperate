...
except (ImportError, AttributeError, OSError) as _pyrender_err:
    render_pc = None
    import logging as _logging

    _logging.getLogger(__name__).warning(
        f"Could not import renderer (pyrender): {_pyrender_err}. "
        "Rendering functions will be unavailable (OK for training)."
    )
...
from graspgenx.dataset.webdataset_utils import GraspWebDatasetReader, is_webdataset
...
class PickDataset(Dataset):
    ...
    def load_cache(self, cache_save_freq: int = 2000):
        ...
        for idx in tqdm(range(self.__len__())):
            ...
            if save_emd_data:
                from graspgenx.dataset.dataset_utils import compute_emd_data
        ...
        if self.load_discriminator_dataset:
            ...
            if not load_contact_batch:
                ...
                outputs["grasps"] = torch.from_numpy(
                    np.array(outputs["grasps"]).astype(np.float)
                )
...
def load_discriminator_batch_with_stratified_sampling(
    ...
    has_offline_pos = grasps_positive is not None and len(grasps_positive) > 0
    has_offline_neg = grasps_negative is not None and len(grasps_negative) > 0
    has_onpolicy_pos = (
        positive_grasps_onpolicy is not None and len(positive_grasps_onpolicy) > 0
    )
    has_onpolicy_neg = (
        negative_grasps_onpolicy is not None and len(negative_grasps_onpolicy) > 0
    )

    include_true_pos = has_offline_pos
    include_true_neg = has_offline_neg
    ...
    if not has_onpolicy_pos:
        num_pos_true_grasps += num_pos_true_onpolicy_grasps
        num_pos_true_onpolicy_grasps = 0

    if not has_onpolicy_neg:
        num_neg_true_grasps += num_neg_true_onpolicy_grasps
        num_neg_true_onpolicy_grasps = 0

    if not has_offline_pos:
        orphaned = num_pos_true_grasps
        if has_onpolicy_pos:
            num_pos_true_onpolicy_grasps += orphaned
        elif has_onpolicy_neg:
            num_neg_true_onpolicy_grasps += orphaned
        elif has_offline_neg:
            num_neg_true_grasps += orphaned
        else:
            num_neg_freespace += orphaned
        num_pos_true_grasps = 0

    if not has_offline_neg:
        orphaned = num_neg_true_grasps
        if has_onpolicy_neg:
            num_neg_true_onpolicy_grasps += orphaned
        elif has_onpolicy_pos:
            num_pos_true_onpolicy_grasps += orphaned
        elif has_offline_pos:
            num_pos_true_grasps += orphaned
        else:
            num_neg_freespace += orphaned
    ...
    if (
        not include_true_neg
        and not include_true_pos
        and not has_onpolicy_pos
        and not has_onpolicy_neg
    ):
        logger.warning(
            "No grasps from any source (offline or on-policy). Returning None."
        )
        return None

    _ts_setup = _time.monotonic()
    ...
    if num_neg_true_onpolicy_grasps > 0:
        ...
        if grasps is None:
            grasps = grasps_neg_true_onpolicy
            grasp_ids = grasps_id_neg_true_onpolicy
        else:
    ...
    if num_neg_freespace > 0:
        ...
        if grasps is None:
            grasps = grasps_neg_freespace
            grasp_ids = grasps_id_neg_freespace
        else:
            grasps = torch.vstack([grasps, grasps_neg_freespace]).float()
            grasp_ids = torch.vstack([grasp_ids, grasps_id_neg_freespace]).int()

    _ts_freespace = _time.monotonic()

    if num_neg_hnretract > 0 and has_offline_pos:
        ...
        if grasps is None:
            grasps = grasps_neg_retract
            grasp_ids = grasps_id_neg_retract
        else:
    ...
    if include_true_neg:
        ...
        if grasps is None:
            grasps = grasps_neg_true
            grasp_ids = grasps_id_neg_true
        else:
    ...
    _ts_end = _time.monotonic()
    _ts_total = _ts_end - _ts0
    if _ts_total > 0.1:
        logger.info(
            f"[STRATIFIED TIMING] total={_ts_total:.4f}s | "
            f"setup={_ts_setup - _ts0:.4f} "
            f"pos_neg_sample={_ts_pos_neg - _ts_setup:.4f} "
            f"freespace={_ts_freespace - _ts_pos_neg:.4f} "
            f"retract={_ts_retract - _ts_freespace:.4f} "
            f"true_neg+labels={_ts_end - _ts_retract:.4f}"
        )
...
def collate_batch_keys(batch):
    ...
    for key in batch:
        ...
        if key in ["contact_dirs", "approach_dirs"]:
