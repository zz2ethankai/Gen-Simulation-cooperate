...
try:
    ...
    try:
        ...
        warnings.warn(
            "Unable to import PointNet++ backbone (pointnet2_ops). "
            "PointNet++ models will not be available. "
            "Other backbones (e.g. WarpConvNet) are unaffected."
        )
        ops = None
...
def _check_ops():
    if ops is None:
        raise RuntimeError(
            "PointNet++ ops (pointnet2_ops) are not available. "
            "Install pointnet2_ops or use a different backbone (e.g. WarpConvNet)."
        )
