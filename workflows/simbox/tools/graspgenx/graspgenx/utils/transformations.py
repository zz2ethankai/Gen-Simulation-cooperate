...
"""
3D transformation utilities: rotation conversions (quaternion / matrix / euler /
axis-angle / 6D), grasp pose (R, t) representations, and alignment helpers.
...
# ---------------------------------------------------------------------------
# Axis-angle <-> matrix, axis-angle <-> quaternion
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Grasp-pose (R, t) flattened representations
# ---------------------------------------------------------------------------
...
def matrix_to_rt(
    ...
    if grasp_repr == "r3_6d":
        ...
        grasps_r = matrix_to_euler_angles(grasps_r_mat, convention="XYZ")
        noise_scale_rot = 1 / torch.pi
        grasps_r = grasps_r * noise_scale_rot  # Scale range to [-1, 1]
    else:
...
def rt_to_matrix(
    ...
    mat = torch.eye(4, device=input.device).unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    if grasp_repr == "r3_6d":
        ...
        mat[:, :3, :3] = euler_angles_to_matrix(
...
def rotation_matrix_from_vectors(
    v1: Union[torch.Tensor, np.ndarray, List],
    v2: Union[torch.Tensor, np.ndarray, List],
) -> torch.Tensor:
    """Compute the rotation matrix that aligns vector v1 onto vector v2.

    Uses the axis-angle representation: the rotation axis is the cross product
    of v1 and v2, and the angle is derived from their dot product. Both inputs
    are normalized to unit vectors first.
    """
